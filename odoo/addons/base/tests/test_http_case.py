...
from crossnow.tests.common import HttpCase, tagged, ChromeBrowser
from crossnow.tools import config, logging
...
class TestHttpCase(HttpCase):
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('crossnow.tests.common.ChromeBrowser.take_screenshot', return_value=None):
    ...
    def test_console_log_object(self):
        logger = logging.getLogger('crossnow')
        level = logger.level
