...
from crossnow.exceptions import AccessDenied, AccessError
from crossnow.http import _request_stack

import crossnow
import crossnow.tools
from crossnow.tests import common
from crossnow.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def test_xmlrpc_defaultdict_marshalling(self):
        ...
        self.patch(self.registry['res.users'], 'context_get',
                   crossnow.api.model(lambda *_: collections.defaultdict(int)))
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, crossnow.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
