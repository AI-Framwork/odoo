...
import crossnow
from crossnow.tests.common import get_db_name, tagged
...
methods_to_sanitize = {
    method_name
    for method_name in dir(crossnow.models.BaseModel)
...
def get_odoo_module_name(python_module_name):
    if python_module_name.startswith('crossnow.addons.'):
        return python_module_name.split('.')[2]
    if python_module_name == 'crossnow.models':
        return 'crossnow'
...
class TestLintOverrideSignatures(LintCase):
    def test_lint_override_signature(self):
        ...
        registry = crossnow.registry(get_db_name())
        ...
        #with open('/tmp/crossnow-override-signatures.md', 'w') as f:
