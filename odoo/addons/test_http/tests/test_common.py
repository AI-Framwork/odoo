...
import crossnow
from crossnow.http import Session
from crossnow.addons.base.tests.common import HttpCaseWithUserDemo
from crossnow.tools.func import lazy_property
from crossnow.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        lazy_property.reset_all(crossnow.http.root)
        cls.addClassCleanup(lazy_property.reset_all, crossnow.http.root)
        cls.classPatch(crossnow.conf, 'server_wide_modules', ['base', 'web', 'test_http'])
        cls.classPatch(crossnow.http.Application, 'session_store', session_store)
        cls.classPatch(crossnow.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(crossnow.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        crossnow.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('crossnow.http.db_list') as db_list, \
             patch('crossnow.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('crossnow.http.db_list') as db_list, \
             patch('crossnow.http.db_filter') as db_filter, \
             patch('crossnow.http.Registry') as Registry:
