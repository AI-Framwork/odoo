...
import crossnow
from crossnow.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in crossnow.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [crossnow.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(crossnow.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be crossnow/studio unless it comes from an imported module install
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(
                data[f.id][0] or "crossnow/studio",
    ...
    def count_database(self, database):
        registry = crossnow.registry(config['db_name'])
        with registry.cursor() as cr:
            uid = crossnow.SUPERUSER_ID
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="CrossNow cloc", lines="Line", other="Other", code="Code")
        s += hr
