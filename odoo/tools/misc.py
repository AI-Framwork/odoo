...
"""
Miscellaneous tools used by CrossNow.
"""
import cProfile
import collections
import contextlib
...
from crossnow.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if crossnow.tools.config['db_host']:
        env['PGHOST'] = crossnow.tools.config['db_host']
    if crossnow.tools.config['db_port']:
        env['PGPORT'] = str(crossnow.tools.config['db_port'])
    if crossnow.tools.config['db_user']:
        env['PGUSER'] = crossnow.tools.config['db_user']
    if crossnow.tools.config['db_password']:
        env['PGPASSWORD'] = crossnow.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',), env=None):
    ...
    addons_paths = crossnow.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None, env=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/crossnow/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with crossnow.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with crossnow.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def scan_languages():
    """ Returns all languages supported by CrossNow for translation
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('crossnow.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('crossnow.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `crossnow.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = crossnow.fields.Datetime.from_string(value)
            value = crossnow.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = crossnow.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = crossnow.fields.Datetime.from_string(value)
    else:
        timestamp = value
