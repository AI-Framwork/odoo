...
class Neutralize(Command):
    ...
    def run(self, args):
        ...
        opt = crossnow.tools.config.parse_config(args)

        dbname = crossnow.tools.config['db_name']
        if not dbname:
        ...
        try:
            with crossnow.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = crossnow.modules.neutralize.get_installed_modules(cursor)
                    queries = crossnow.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    crossnow.modules.neutralize.neutralize_database(cursor)
