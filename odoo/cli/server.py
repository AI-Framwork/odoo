...
"""
CrossNow - Server
CrossNow is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - CrossNow SA
"""
...
__author__ = crossnow.release.author
__version__ = crossnow.release.version

# Also use the `crossnow` logger for the main script.
_logger = logging.getLogger('crossnow')

re._MAXCACHE = 4096  # default is 512, a little too small for crossnow
...
def report_configuration():
    ...
    """
    config = crossnow.tools.config
    _logger.info("CrossNow version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', crossnow.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = crossnow.tools.config
    if not crossnow.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = crossnow.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            crossnow.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = crossnow.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = crossnow.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    crossnow.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                crossnow.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except crossnow.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        crossnow.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = crossnow.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the crossnow server (default command)"""
    def run(self, args):
