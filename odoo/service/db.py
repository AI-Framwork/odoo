...
import crossnow.sql_db
import crossnow.tools
from crossnow.sql_db import db_connect
from crossnow.release import version_info
from crossnow.tools import find_pg_tool, exec_pg_environ
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not crossnow.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and crossnow.tools.config.verify_admin_password(passwd):
        return True
    raise crossnow.exceptions.AccessDenied()

# This should be moved to crossnow.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            crossnow.tools.config['load_language'] = lang
            cr.commit()

        registry = crossnow.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = crossnow.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = crossnow.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = crossnow.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = crossnow.tools.config['db_template']
    ...
    try:
        db = crossnow.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if crossnow.tools.config['unaccent']:
                ...
                # But in the case of CrossNow, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    crossnow.sql_db.close_db(db_original_name)
    db = crossnow.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = crossnow.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = crossnow.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            crossnow.modules.neutralize.neutralize_database(cr)

    from_fs = crossnow.tools.config.filestore(db_original_name)
    to_fs = crossnow.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    crossnow.modules.registry.Registry.delete(db_name)
    crossnow.sql_db.close_db(db_name)
    ...
    fs = crossnow.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'odoo_dump': '1',
        'db_name': cr.dbname,
        'version': crossnow.release.version,
        'version_info': crossnow.release.version_info,
        'major_version': crossnow.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = crossnow.tools.config.filestore(db_name)
            ...
            if stream:
                crossnow.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                crossnow.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = crossnow.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = crossnow.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    crossnow.modules.registry.Registry.delete(old_name)
    crossnow.sql_db.close_db(old_name)
    ...
    old_fs = crossnow.tools.config.filestore(old_name)
    new_fs = crossnow.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    crossnow.tools.config.set_admin_password(new_password)
    crossnow.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        crossnow.tools.config['update']['base'] = True
        crossnow.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@crossnow.tools.mute_logger('crossnow.sql_db')
...
def list_dbs(force=False):
    if not crossnow.tools.config['list_db'] and not force:
        raise crossnow.exceptions.AccessDenied()

    if not crossnow.tools.config['dbfilter'] and crossnow.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, CrossNow will not
        ...
        res = sorted(db.strip() for db in crossnow.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [crossnow.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of CrossNow
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if crossnow.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        crossnow.sql_db.close_db(database_name)
...
def exp_list_lang():
    return crossnow.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(crossnow.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return crossnow.release.version
