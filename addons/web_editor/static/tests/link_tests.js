/** @crossnow-module **/
import { setSelection } from "@web_editor/js/editor/crossnow-editor/src/utils/utils";
...
} from "@web_editor/js/editor/crossnow-editor/test/utils";
...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test("typing valid URL + space should convert to link", async function (assert) {
                ...
                const node = editable.querySelector("p");
                setSelection(node, 0);
                inputText(".crossnow-editor-editable p", "http://google.co.in");
                insertText(editor, " ");
                editor.clean();
                assert.strictEqual(
                    editable.innerHTML,
        ...
        QUnit.test("typing invalid URL + space should not convert to link", async function (assert) {
            ...
            const node = editable.querySelector("p");
            setSelection(node, 0);
            inputText(".crossnow-editor-editable p", "www.crossnow");
            insertText(editor, " ");
            editor.clean();
            assert.strictEqual(editable.innerHTML, "<p>www.crossnow </p>");
        });
