...
import { getAdjacentPreviousSiblings, isBlock, rgbToHex, commonParentGet } from '../editor/crossnow-editor/src/utils/utils';
...
export async function toInline($editable, cssRules, $iframe) {
    $editable.removeClass('crossnow-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('crossnow-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const { fonts } = crossnow.loader.modules.get("@web_editor/js/wysiwyg/fonts");
