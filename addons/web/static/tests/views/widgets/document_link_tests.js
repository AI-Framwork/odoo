    ...
    QUnit.test("documentation_link: absoluth path (http)", async function (assert) {
        await makeView({
            ...
            arch: `
                <form>
                    <field name="bar"/>
                    <widget name="documentation_link"  path="http://www.crossnow.com/"/>
                </form>`,
        });

        assert.hasAttrValue(target.querySelector(".o_doc_link"), "href", "http://www.crossnow.com/");
    });
    QUnit.test("documentation_link: absoluth path (https)", async function (assert) {
        await makeView({
            ...
            arch: `
                <form>
                    <field name="bar"/>
                    <widget name="documentation_link"  path="https://www.crossnow.com/"/>
                </form>`,
        });

        assert.hasAttrValue(target.querySelector(".o_doc_link"), "href", "https://www.crossnow.com/");
    });
});
