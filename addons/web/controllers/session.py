...
from crossnow.modules import module
from crossnow.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable CrossNow Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        if pre_uid != request.session.uid:
            # Crapy workaround for unupdatable CrossNow Mobile App iOS (Thanks Apple :@) and Android
        ...
        registry = crossnow.modules.registry.Registry(db)
        with registry.cursor() as cr:
            env = crossnow.api.Environment(cr, request.session.uid, request.session.context)
            if not request.db:
    ...
    def account(self):
        ...
        return 'https://accounts.crossnow.com/oauth2/auth?' + url_encode(params)
