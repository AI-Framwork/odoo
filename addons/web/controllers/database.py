...
from crossnow.http import content_disposition, dispatch_rpc, request, Response
from crossnow.service import db
from crossnow.tools.misc import file_open, str2bool
from crossnow.tools.translate import _

from crossnow.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = crossnow.tools.config.verify_admin_password('admin')
        d['list_db'] = crossnow.tools.config['list_db']
        d['langs'] = crossnow.service.db.exp_list_lang()
        d['countries'] = crossnow.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = crossnow.service.db.list_db_incompatible(d['databases'])
        except crossnow.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = crossnow.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            crossnow.service.db.check_super(master_pwd)
            ...
            dump_stream = crossnow.service.db.dump_db(name, None, backup_format)
